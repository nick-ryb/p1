...
function ChildView(location) {
  ...
  const _childActivityToday = (info, lang) => {
    if (!info || !info.activity) return null;
        ...
        return (
          <div key={"getSleepWrap"} className={"activity"}>
            ...
            <div className={"sleepRangesList"}>
              {sleep.map((f, i) => {
                return (
                  <div key={`sleepRange${i}wrap`} className={"sleepRange"}>
                    <span key={`sleep${i}from`}>{`${getLabel(lang, "from")} ${
                      f[0]
                    } `}</span>
                    <span key={`sleep${i}till`}>{` ${getLabel(lang, "till")} ${
                      f[1]
                    }`}</span>
                  </div>
                );
    ...
    return [getSleep(), getFood(), getPoop()];
  };
  ...
  const _renderChildActivityView = lang => {
    ...
    return (
      <div className={"current_activities_view"}>
        {todayActivity ? (
          this._childActivityToday(todayActivity, lang)
        ) : (
  ...
  return [
    <p>{`222`}</p>,
    <Route path="/Child" render={() => <div>Route</div>} />
  ];
}
